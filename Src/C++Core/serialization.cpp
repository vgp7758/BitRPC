...
// NetworkFrameHandler implementation
#ifdef _WIN32
#include <winsock2.h>
#include <ws2tcpip.h>
#else
...
std::vector<uint8_t> NetworkFrameHandler::read_frame(void* socket) {
    ...
    if (bytes_received <= 0) {
        throw std::runtime_error("Connection closed while reading frame length");
    }

    if (bytes_received != sizeof(frame_length)) {
        throw std::runtime_error("Failed to read complete frame length");
    }

    // Check for special markers
    if (frame_length == STREAM_END_MARKER || frame_length == ERROR_MARKER) {
        std::vector<uint8_t> marker_data(sizeof(uint32_t));
        *reinterpret_cast<uint32_t*>(marker_data.data()) = frame_length;
        return marker_data;
    }

    // Read frame data
    std::vector<uint8_t> frame_data(frame_length);
    size_t total_received = 0;

    while (total_received < frame_length) {
        bytes_received = recv(sock, reinterpret_cast<char*>(frame_data.data() + total_received),
                              static_cast<int>(frame_length - total_received), 0);

        if (bytes_received <= 0) {
            throw std::runtime_error("Connection closed while reading frame data");
        }
...
bool NetworkFrameHandler::write_frame(void* socket, const std::vector<uint8_t>& data) {
    SOCKET sock = reinterpret_cast<SOCKET>(socket);
...
bool NetworkFrameHandler::write_end_marker(void* socket) {
    SOCKET sock = reinterpret_cast<SOCKET>(socket);
    uint32_t end_marker = STREAM_END_MARKER;
    int bytes_sent = send(sock, reinterpret_cast<const char*>(&end_marker), sizeof(end_marker), 0);
    return bytes_sent == sizeof(end_marker);
}

bool NetworkFrameHandler::write_error_marker(void* socket, const std::string& error_msg) {
    SOCKET sock = reinterpret_cast<SOCKET>(socket);

    // Write error marker
    uint32_t error_marker = ERROR_MARKER;
    int bytes_sent = send(sock, reinterpret_cast<const char*>(&error_marker), sizeof(error_marker), 0);

    if (bytes_sent != sizeof(error_marker)) {
        return false;
    }

    // Write error message length
    uint32_t msg_length = static_cast<uint32_t>(error_msg.size());
    bytes_sent = send(sock, reinterpret_cast<const char*>(&msg_length), sizeof(msg_length), 0);
    ...
    // Write error message
    bytes_sent = send(sock, error_msg.c_str(), static_cast<int>(error_msg.size()), 0);

    return bytes_sent == static_cast<int>(error_msg.size());
}
