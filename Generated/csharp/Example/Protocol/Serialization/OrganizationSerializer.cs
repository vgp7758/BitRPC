...
namespace Example.Protocol.Serialization
{
    public class OrganizationSerializer : ITypeHandler
    {
        ...
        public void Write(object obj, BitRPC.Serialization.StreamWriter writer)
        {
            ...
            if (mask.GetBit(1))
            {
                writer.WriteList(message.groups, x => GroupSerializer.Write(x, writer));
            }
            if (mask.GetBit(2))
            {
            ...
            if (mask.GetBit(3))
            {
                UserSerializer.Write(message.leader, writer);
            }
            }
            finally
            {
        ...
        public object Read(BitRPC.Serialization.StreamReader reader)
        {
            ...
            if (masks[0].GetBit(1))
            {
                message.groups = reader.ReadList(() => GroupSerializer.ReadStatic(reader));
            }
            if (masks[0].GetBit(2))
            {
            ...
            if (masks[0].GetBit(3))
            {
                message.leader = UserSerializer.ReadStatic(reader);
            }
            return message;
            }
            finally
        ...
        private static readonly OrganizationSerializer _instance = new OrganizationSerializer();

        public static void Write(Organization obj, BitRPC.Serialization.StreamWriter writer)
        {
            _instance.Write(obj, writer);
        }

        public static Organization ReadStatic(BitRPC.Serialization.StreamReader reader)
        {
            return (Organization)_instance.Read(reader);
        }
